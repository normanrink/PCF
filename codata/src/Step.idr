...
---------------------------------------------------------
-- Begin: TRANSITIVE CLOSURES: INDUCTIVE AND CO-INDUCTIVE
...
-- Codata type for representing the transitive closure of the 'Step' relation:
codata CoTransStep : Term [] t -> Term [] t -> Type where
   CoTStRefl   : (e : Term [] t) -> CoTransStep e e
   CoTStTrans  : {e : Term [] t} -> {e' : Term [] t} -> {e'' : Term [] t} ->
                 Step e e' -> CoTransStep e' e'' -> CoTransStep e e''


coTransStepTransitive : CoTransStep e1 e2 -> CoTransStep e2 e3 -> CoTransStep e1 e3
coTransStepTransitive (CoTStRefl _)        ctst  = ctst
coTransStepTransitive (CoTStTrans st ctst) ctst' = 
  CoTStTrans st $ coTransStepTransitive ctst ctst'


transToCoTrans : TransStep e1 e2 -> CoTransStep e1 e2
transToCoTrans (TStRefl e1)      = CoTStRefl e1
transToCoTrans (TStTrans st tst) = CoTStTrans st $ transToCoTrans tst

-- End: TRANSITIVE CLOSURES: INDUCTIVE AND CO-INDUCTIVE
-------------------------------------------------------
